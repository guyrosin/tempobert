...
def concat(texts, time_ids, stringify_time_id=noop):
...
def concat_tuples(text_pairs, time_id_pairs, stringify_time_id=noop):
    """Prepend the time to each text, where the inputs are pairs"""
    texts = [
        concat(text_pair, time_id_pair, stringify_time_id)
...
class TempoPreTrainedTokenizerFast(TempoPreTrainedTokenizerBase):
    ...
    def _batch_encode_plus(
        ...
        if self.time_embedding_type.startswith("prepend_") and batch_time_id:
            kwargs = {}
            ...
            if utils.check_type(batch_text_or_text_pairs, List[TextInput]):
                texts = concat(batch_text_or_text_pairs, batch_time_id, **kwargs)
            elif utils.check_type(batch_text_or_text_pairs, List[TextInputPair]):
                texts = concat_tuples(batch_text_or_text_pairs, batch_time_id, **kwargs)
            else:
